...
| `--no-fill` | — | 余剰語数の再配分を止め、§4 の素の配分だけを使う |
...
`--budget` に対する総語数は、**90〜190 words の範囲では ±10% に収まる**（既定 150）。
これは M0〜M3 が long / mid / short の3段しか持たないためで、
語彙の最短構成（約 60 words）より小さい予算、最長構成（約 230 words）より大きい予算では
±10% に入りようがない。その場合は**必ず配分表に警告行を出す**。黙って外すことはしない。
...
4. 超過していれば M3 → M2 → M1（→ M0）の順に1段ずつ落とす。subject は最後まで落とさない
5. **（拡張）** 余った語数で1段ずつ上げ直す。それでも下限90%に届かない時だけ、
   上限110%を超えない範囲でもう1段上げる。`--no-fill` で 4 まででやめられる
...
5. **§4 の余剰再配分** — 上記「語数予算の実際」の 5。
   SUCCESS_CRITERIA「出力語数が指定予算の ±10%」を満たすために追加した。
...
- **柄語彙に色名が埋め込まれている。** §1-1 の語彙には `amber dot grid` /
  `black tartan` / `narrow ochre pinstripe` のように色名を含むものがある。
  配色は §1-2 の明度差から独立に決まるため、
  `amber dot grid, deep crimson on ecru` のような組み合わせが出る。
  §5 の排他グループに色語の項が無いため lint は通す。
  語彙側を直すか、そのまま使うかは人間の判断に属する（この道具は判定しない）。
